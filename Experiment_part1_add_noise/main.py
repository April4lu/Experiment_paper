...
from get_grad_train import DpGradientDescentOptimizer_noise
...
def train(mnist):
    ...
    with tf.name_scope('Optimizer'):
        ...
        train_op = DpGradientDescentOptimizer_noise(learning_rate,loss,N_vars,global_step)
        
    with tf.name_scope('Accuracy'):
        #定义准确率
        correct_prediction = tf.equal(tf.argmax(y,1),tf.argmax(y_,1))
        accuracy = tf.reduce_mean(tf.cast(correct_prediction,tf.float32))
    save_file = './nn_noise/train_model.ckpt1'
